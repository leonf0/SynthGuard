...
from ..config import DEVICE, N_REGIMES, SEQUENCE_LENGTH
...
def train_diffusion(
    ...
    optim_ = torch.optim.AdamW(model.denoiser.parameters(), lr=lr)
    ...
    for epoch in range(epochs):
        ...
        for x0, reg in loader_train:
            ...
            model.train_step(x0, reg, optim_)
        scheduler.step()
        ...
        if avg_val < best_val_loss:
            ...
            best_state = {
                k: v.cpu().clone() for k, v in model.denoiser.state_dict().items()
            }
